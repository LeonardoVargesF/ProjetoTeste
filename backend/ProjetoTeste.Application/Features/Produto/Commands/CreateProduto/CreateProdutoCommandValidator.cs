...
namespace ProjetoTeste.Application.Features.Produto.Commands.CreateProduto
{
    public class CreateProdutoCommandValidator : AbstractValidator<CreateProdutoCommand>
    {
        ...
        public CreateProdutoCommandValidator(IProdutoRepository produtoRepository)
        {
            RuleFor(p => p.Codigo)
        ...
        private async Task<bool> ProdutoCodBarrasUnique(CreateProdutoCommand command, CancellationToken token)
        {
            return await _produtoRepository.IsCodBarrasUnique(command.CodBarras);
        }

        private async Task<bool> ProdutoCodigoUnique(CreateProdutoCommand command, CancellationToken token)
        {
